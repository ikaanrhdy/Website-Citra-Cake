...
const DibatalkanPage = () => {
  ...
  // parse string "DD/MM/YYYY" (format tanggalPengiriman di Order) jadi Date,
  // gak pakai new Date(string) langsung soalnya itu keparsing MM/DD/YYYY (ambigu)
  const parseTanggalIndo = (value: string): Date | undefined => {
  ...
  const handleBeliLagi = (order: (typeof orders)[number]) => {
    // tanggal kirim lama cuma dipakai kalau masih valid (>= hari ini),
    // kalau udah lewat biarin kosong biar user pilih ulang di Checkout
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);

    const oldTanggalPengiriman = parseTanggalIndo(order.tanggalPengiriman);

    const tanggalKirimValid =
      oldTanggalPengiriman && oldTanggalPengiriman >= todayStart
        ? oldTanggalPengiriman.toISOString()
        : undefined;
    ...
    navigate("/checkout", {
      state: {
        ...
        // optional, cuma prefill kalau datanya ada
        alamat: order.alamat ?? "",
      ...
      {orders.map((order) => (
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3 md:gap-4">
            {/* GAMBAR - 1/3 */}
            <div className="col-span-1 flex items-start justify-center p-1 rounded border border-gray-300 shrink-0">
              <img
